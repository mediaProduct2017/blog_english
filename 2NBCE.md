...
Because the self attention matrix for transformer is $L \times L$, the memory cost will be quadratic as L increases. However, the time cost will not be quadratic if L is not very large (e.g. larger than 10k or 100k), because of parallel computing of GPU. If L is very large, all the parallel computing capacity will be occupied so that more time is needed and time complexity becomes quadratic.
